...
如果我们用标量、向量或矩阵描述一个事物时，该事物最多可用 [H,W]的维度表示。在现实与客观世界中，我们经常会碰到的物体的维度可能会更高维度，很难通过向量或矩阵来描述，这时我们就需要**Tensor（张量）**。

我们可以通过张量来描述任意维度的物体 `H * W * C`，其中C为C维的特征图，除外还可以描述更高维度的物体：`H * W * C* D`，其中D为未知的更高维空间。对于这样的维度的数据描述，我们都可以采用**张量(Tensor)**来进行描述。

> 特征图是深度学习中的一个概念。



总之，张量是对于标量、向量、矩阵之上进行更加泛化的定义。标量可以看成是0阶的张量，向量是1阶段张量，矩阵是2阶的张量，除了`0,1,2阶`的张量之外，还有`3阶、4阶、5阶..n阶`的张量。**深度学习中** 数据输入的维度是不确定的（可以是任意维度），这时就需要采用一个更加广泛的概念去描述这些量，Tensor就可以更便利解决该问题的。
...
**样本**（机器学习中用到的数据）就可通过Tensor来对其进行描述。

**比如一条语音数据**，我们可有能会采用向量来进行描述，该向量就是1阶的张量。向量描述采样过之后在当前时刻的声音特征，图形化之后为波行。对于灰度图，我们通常采用矩阵描述（二阶Tensor），表示成`H*W`，对于彩色图会描述成`[H * W * C]`，为一个三阶Tensor，其中C=3。



而**模型**（分类有参数模型与无参数模型)，有参数模型一般被描述为  `Y = WX + b`函数，其中X是指样本，W,B是指参数。当W与B未知的情况下为变量，该变量也是通过Tensor来表示，Y为最后的标签，而标签在进行数字化时也会通过Tensor来对其进行描述。
...
![image-20240102201942723](1.PyTorch零基础入门/image-20240102201942723.png)
...
- 操作：切片、索引、变型等
- numpy：Tensor可以与numpy互换（numpy也可以用来描述样本与模型）

https://blog.nianxi.cc/article/2348287033.html)



### Tensor的类型 - torch.dtype

张量（Tensor）是Pytorch库中的基本数据类型，Pytorch中各种基本数字类型都有其对应的Tensor类型（但是在Pytorch中没有内嵌的字符串类型），Tensor的每种类型分别有对应CPU和GPU版本。默认的Tensor是FloatTensor，可通过**```torch.set_default_tensor_type(类型名)```**修改默认tensor类型（如果默认类型为GPU tensor，则所有操作都将在GPU上进行）。



Tensor的类型如下表，加粗部分为常用类型：
...
- **查看张量数据类型的方法**
  - **type方法**：使用`张量名.type()`的方式可以查看张量的具体类型。
  - isinstance函数：isinstance是Python的自带函数，语法为`isinstance(张量名,类型名)`，返回值是一个布尔值。`isinstance(torch.randn(2,3),torch.FloatTensor)`
...
**`torch.sparse_coo_tensor`参数的具体作用**：
